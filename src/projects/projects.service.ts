...
import { CreateProjectDto, FindProjectDto, UpdateProjectDto, UpdateProjectVacantion } from './projects.dto'

@Injectable()
export class ProjectsService {
  constructor(
    @InjectRepository(Project) private readonly projectRepository: Repository<Project>,
    @InjectRepository(Vacantion) private readonly vacantionRepository: Repository<Vacantion>,
  ) {}
  ...
  async findOne(where: FindProjectDto) {
    return this.projectRepository.findOne(where, { relations: ['user', 'vacantions'] })
  }
  ...
  async insertVacantions(data: UpdateProjectVacantion[], project: Project) {
    const vacantions = await this.vacantionRepository.find({ projectId: project.id })
    const vacantionIdsForDelete = vacantions
      .filter(existingVacation => data.findIndex(newVacantion => existingVacation.id === newVacantion.id) === -1)
      .map(vacantion => vacantion.id)
    if (vacantionIdsForDelete.length) {
      this.vacantionRepository.delete(vacantionIdsForDelete)
    }

    const newVacantions = data.filter(vacantion => !vacantion.id).map(vacantion => this.vacantionRepository.create({ ...vacantion, project }))
    const updatedVacations = data.filter(vacantion => vacantion.id).map(vacantion => this.vacantionRepository.create({ ...vacantion, project }))

    return this.vacantionRepository.save([...newVacantions, ...updatedVacations])
  }
